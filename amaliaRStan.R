...
## To work in RStan we need to invert the function and sample from this hazard rate. The following function can be used to find the inverse of a function. Our function should involve inserting a beta and gamma (lambda) then returning "T_real", putting it in an optimiser, which will then spit out approximately the gammas and beta (lambda) we used earlier.
